...
set-konsole-tab-title ()
{
    ...
    # Validate input
    if [[ -z "${_title}" ]]; then
        echo "Usage: ${FUNCNAME[0]} <title>"
        echo "  title: Tab title to set"
        return 1
    fi
    
    # Validate Konsole environment
    if [[ -z "${KONSOLE_DBUS_SERVICE}" ]] || [[ -z "${KONSOLE_DBUS_SESSION}" ]]; then
        echo "Error: Not running in a Konsole terminal"
        return 1
    fi
    # Set the tab title using qdbus
    qdbus6 >/dev/null "${KONSOLE_DBUS_SERVICE}" "${KONSOLE_DBUS_SESSION}" setTitle 1 "${_title}"
}
